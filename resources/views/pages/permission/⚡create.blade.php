...
new class extends Component {
    ...
    public function pages()
    {
        return Permission::query()
            ->pluck('name')
            ->map(fn (string $name) => str($name)->before('.'))
            ->unique()
            ->sort()
            ->values();
    }
    ...
    <main class="flex-1 overflow-y-auto px-4 sm:px-6 py-6 space-y-6">
        <div class="w-full">
            ...
            <div class="flex items-start justify-between gap-4">
                <div>
                    <h1 class="font-display text-2xl font-medium tracking-tight text-ink">Create permission</h1>
                    <p class="mt-1 text-sm text-mist">Add a new permission to the application</p>
                </div>
            ...
            <form wire:submit="save" class="mt-6 rounded-xl border border-line bg-card p-6 space-y-5"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input id="page" type="text" wire:model="page" list="permission-pages"
                            placeholder="Select or create new page…" class="field-input" />
                        <datalist id="permission-pages">
